...
namespace TableEntityGenerator.Core
{
    public class GeneratorCore
    {
        ...
        public async Task<IEnumerable<(string fileName, string text)>> GenerateSourceFileAsync(string nameSpace)
        => (await _repository.ListAllAsync()).Select(t => (
            $"{t.Name}.cs",
            ((ITextTemplate)new TableEntity(_repository.TypeMapping, nameSpace, t)).TransformText()
        ));
    }
}